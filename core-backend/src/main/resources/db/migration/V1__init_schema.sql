...
-- Core Asset Registry metadata lookup
...
-- Optimize matching lookups during streaming joins
CREATE INDEX IF NOT EXISTS idx_grid_assets_geo ON grid_assets (latitude, longitude);

-- Target table for TimescaleDB hypertable tracking raw historical anomaly points
CREATE TABLE IF NOT EXISTS spatial_anomaly_alerts (
...
SELECT create_hypertable('spatial_anomaly_alerts', 'timestamp', if_not_exists => TRUE);

CREATE INDEX IF NOT EXISTS idx_spatial_anomaly_alerts_household_time
    ON spatial_anomaly_alerts (household_id, timestamp DESC);
